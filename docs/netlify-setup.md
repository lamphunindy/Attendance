...
หน้าจัดการโหลดเฉพาะตัวเลือกที่แบบฟอร์มและตารางในหน้านั้นใช้ เช่น หน้ารายวิชาไม่โหลดชุดตัวเลือกใดเลย ส่วนหน้านักเรียนและนำเข้าโหลดเฉพาะห้องเรียนกับปีการศึกษา การตรวจ session และสถานะย้ายข้อมูลเริ่มพร้อมกัน และยังตรวจการเพิกถอน session ทุกคำขอตามเดิม

เมื่อขั้นตอนใดใช้เวลาอย่างน้อย 1 วินาที จะมี `[performance]` ใน Function log พร้อม `stage`, `duration_ms` และ AWS `region` เพื่อแยกเวลาตรวจบัญชี (`auth.verify-session`), อ่านสถานะย้ายข้อมูล (`auth.migration`), อ่านโปรไฟล์/สิทธิ์ (`auth.account`) และโหลดข้อมูลหน้า (`page.dashboard`, `page.assignment`, `page.admin-options`) ตัวเลขของแต่ละหน้าอาจรวมเวลาตรวจบัญชี จึงไม่ควรนำทุกบรรทัดมาบวกกัน Log นี้ไม่มีชื่อ อีเมล token หรือข้อมูลนักเรียน และไม่ครอบคลุมเวลาบูตฟังก์ชันก่อนโค้ดเริ่มทำงานหรือการโหลด JavaScript ในเบราว์เซอร์
